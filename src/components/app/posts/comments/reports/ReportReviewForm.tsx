...
import { useReviewCommentReport } from "@/queries/app/post/comment-report-review";
...
export function ReportReviewForm({
  ...
  const reviewPostReport = useReviewCommentReport();
  const form = useForm({
    mode: "controlled",
    initialValues: {
      comment:
  ...
  const handleSubmit = form.onSubmit(async (values) => {
    try {
      ...
      notifications.show({
        title: "Failed to review comment report",
